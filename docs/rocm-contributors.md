...
|---|---|---|
| dockylf | [PR #10](https://github.com/kvmem/kvmem-llama.cpp/pull/10), head `b183c3a` | Built the Windows HIP foundation: the backend-portable adapter abstraction in `llama-kvmem-gpu.h` (+78 lines) that turned hard-coded CUDA runtime calls into a pluggable interface — the ground work that made any non-CUDA backend possible; the `-DGGML_HIP=ON` build backend; Windows host-library build fixes; Windows environment-variable semantics fix; the Windows ROCm/HIP build guide and helpers; server smoke checks. |
| FangJiangyi | [PR #33](https://github.com/kvmem/kvmem-llama.cpp/pull/33), head `a7460f3` | Brought up Linux ROCm end to end: native HIP compilation of the stage-in sources (+620/-399) with a 146-line ROCm memory canary; enabled MTP GDN replay on a non-CUDA backend for the first time; multimodal benchmark tuning with a 192-line recommended-config/performance document containing the 7900 XTX measurements; cumulative-patch factory-header fix; CPU vision thread propagation. |
| Zeshen | Integrated branch | Cross-platform unification and HIP hardening: fixed the GDN fold kernel's hard-coded 32-lane partition so HIP uses the device wave width while CUDA stays at 32 lanes; isolated CUDA toolkit discovery to the CUDA backend; unified cross-platform build entry `build-rocm.py` with automatic GPU-architecture detection (incl. WSL DXG) and build unit tests; strengthened the replay regression test (+147 lines: FP64 reference, record-buffer pollution rejection, production layouts, multi-layer fold); ported the multimodal canary to Windows (639 lines, psapi WorkingSet/Private Bytes sampling); dual-platform IQ3 launchers, relocatable Linux runtime libs, EOL-safe source export, VRAM telemetry fix; validated full 256K 33-round runs on both Windows and Linux. |
